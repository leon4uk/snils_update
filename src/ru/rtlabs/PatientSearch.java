...
public class PatientSearch {
    ...
    public void hasDoc1(DBWorker connection){
        ...
        String sql = "SELECT id, number, issue_dt FROM pim_individual_doc where indiv_id = ? and type_id = 19";
        ...
        try {
            ...
            if (!resultSet.wasNull()){
                while (resultSet.next()){
                    oldDocNumber1 = resultSet.getString(2);
                    oldDocDate1 = resultSet.getDate(3);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void docUpdate1(String number, DBWorker connection){
        String sql = "UPDATE pim_individual_doc SET number = ? WHERE indiv_id = ? and type_id = 19";
        try {
            PreparedStatement preparedStatement = connection.getConnection().prepareStatement(sql);
            preparedStatement.setString(1, number);
            preparedStatement.setInt(2, this.id);
            preparedStatement.execute();
            System.out.println(java.util.Calendar.getInstance().getTime() + " у  пациента обновлены данные по документу СНИЛС. Номер c " + this.oldDocNumber1 + " на " + number);
            FileWriter fileWriter = new FileWriter("log.txt", true);
            BufferedWriter writer = new BufferedWriter(fileWriter);
            writer.write(java.util.Calendar.getInstance().getTime() + " у  пациента обновлены данные по документу СНИЛС. Номер c " + this.oldDocNumber1 + " на " + number);
            writer.newLine();
            writer.flush();
            writer.close();
    ...
    public void docInsert1(String number, DBWorker connection){
        String sql = "INSERT INTO pim_individual_doc (id, type_id, number, indiv_id) values(nextval('pim_individual_doc_id_seq'), ?, ?, ?)";
        try {
            PreparedStatement preparedStatement = connection.getConnection().prepareStatement(sql);
            ...
            preparedStatement.execute();
            System.out.println(java.util.Calendar.getInstance().getTime() + " пациенту добавлен документ СНИЛС с данными " + number );
            FileWriter fileWriter = new FileWriter("log.txt", true);
            BufferedWriter writer = new BufferedWriter(fileWriter);
            writer.write(java.util.Calendar.getInstance().getTime() + " пациенту добавлен документ СНИЛС с данными " + number );
            writer.newLine();
            writer.flush();
            writer.close();
